...
Just a quick heads-up that your MiiTV subscription is due to expire on [date].
...
Your MiiTV subscription expired on [date].
...
Your MiiTV subscription is coming up for renewal.
...
function EmailTemplates({ urgent, subscribers = [], onSend }) {
  ...
  // Replace [Name] and [date] with real values
        ...
        <div style={{ background:'rgba(96,165,250,.04)', border:'1px solid rgba(96,165,250,.15)', borderRadius:12, padding:18, marginBottom:16 }}>
          ...
          <div style={{ marginBottom:14 }}>
            <label style={{ display:'block', fontSize:11, color:'#475569', fontWeight:700, marginBottom:5 }}>
              Body <span style={{ color:'#334155', fontWeight:400 }}>— use [Name] and [date] as placeholders</span>
            </label>
...
export default function MiiTVCRM({ user }) {
  ...
  const API_BASE = 'https://miitv-crm.vercel.app'
  ...
  const contacts = useMemo(() => subscribers.map((s, i) => ({
    ...
    daysLeft: Math.round((new Date(s.expiration) - TODAY) / 86400000),
  ...
  async function syncSheet() {
    ...
    try {
      ...
      if (data.success) {
        setSyncMsg(`✓ Synced ${data.synced} subscribers`)
        await loadAll()
      } else {
      ...
      <div className="topbar" style={{ display:'flex',alignItems:'center',padding:'0 22px',height:54,borderBottom:'1px solid rgba(255,255,255,.06)',background:'rgba(7,9,15,.97)',position:'sticky',top:0,zIndex:100,backdropFilter:'blur(12px)',gap:10 }}>
        ...
        <div className="topbar-actions" style={{ display:'flex',alignItems:'center',gap:8 }}>
          ...
          <div title={user?.email} style={{ width:28,height:28,borderRadius:'50%',background:'linear-gradient(135deg,#f59e0b,#ef4444)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:11,fontWeight:700,color:'#fff',cursor:'pointer' }}
            onClick={signOut}>{(user?.email||'?')[0].toUpperCase()}</div>
        </div>
            ...
            <div className="fu">
              <div className="stat-grid" style={{ display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:11,marginBottom:18,gridTemplateRows:'auto' }}>
                {[
                  ...
                  { label:'Expiring ≤30d',value:expiring,color:'#f59e0b',icon:'⏳' },
                  { label:'Expired',value:expired,color:'#f87171',icon:'❌' },
                  { label:'Multi-device',value:multi,color:'#a78bfa',icon:'🔗' },
                ].map(s=>(
                  <div key={s.label} className="card" style={{ padding:'13px 15px' }}>
                    <div style={{ fontSize:10.5,color:'#475569',fontWeight:700,textTransform:'uppercase',letterSpacing:'.06em',marginBottom:5 }}>{s.icon} {s.label}</div>
          ...
          {view === 'settings' && (
            <div className="fu">
              ...
              <div className="settings-grid" style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:16 }}>
                ...
                <div className="card">
                  <h3 style={{ fontSize:13,color:'#64748b',fontWeight:700,marginBottom:16 }}>🔑 Change Password</h3>
                  <ChangePasswordForm />
                </div>
        ...
        return (
          <Modal title="✉️ Compose Email" onClose={()=>{ setModal(null); setForm({}); setModalTemplates([]) }} wide>
            ...
            <div style={{ display:'flex', gap:6, marginBottom:16, background:'rgba(255,255,255,.03)', borderRadius:9, padding:4 }}>
              {[['individual','👤 Individual'],['bulk','👥 Group / Bulk']].map(([mode,lbl]) => (
                <button key={mode}
                  onClick={() => setForm(f => ({ ...f, sendMode: mode, bulkGroup: mode==='individual'?undefined:f.bulkGroup }))}
              ...
              <div style={{ marginBottom:12 }}>
                ...
                <input
                  ...
                  onChange={e => {
                    ...
                    if (match) setForm(f => ({ ...f, emailTo: match.email, emailToName: match.username, emailToSearch: match.email }))
                  }}
                  style={{ width:'100%',background:'rgba(255,255,255,.05)',border:'1px solid rgba(255,255,255,.1)',borderRadius:8,padding:'9px 12px',color:'#dde4f0',fontFamily:'inherit',fontSize:13,outline:'none' }}
                />
            ...
            <div style={{ marginBottom:12 }}>
              <label style={{ display:'block',fontSize:11,color:'#475569',fontWeight:700,marginBottom:5 }}>
                Message {form.sendMode==='bulk' && <span style={{color:'#334155',fontWeight:400}}>— [Name] and [date] auto-filled per recipient</span>}
              </label>
