...
export default function LoginPage() {
  ...
  async function handleMagicLink(e) {
    ...
    const { error } = await supabase.auth.signInWithOtp({ email, options: { emailRedirectTo: 'https://miitv-crm.vercel.app/dashboard' } })
  ...
  async function handleReset(e) {
    ...
    setLoading(true); setError(''); setSuccess('')
    const { error } = await supabase.auth.resetPasswordForEmail(email, { redirectTo: 'https://miitv-crm.vercel.app/dashboard' })
    if (error) setError(error.message)
    else setSuccess('Reset email sent to ' + email)
    setLoading(false)
  }
  ...
  const lbtn = { background:'none', border:'none', color:'#60a5fa', fontSize:12, cursor:'pointer', fontFamily:'inherit', display:'block', marginTop:8 }
      ...
      <div style={{ width:380, padding:36, background:'rgba(255,255,255,.03)', border:'1px solid rgba(255,255,255,.08)', borderRadius:20 }}>
        <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:28 }}>
          ...
          <div>
            ...
            <div style={{ fontSize:12,color:'#475569' }}>{mode==='password'?'Sign in':mode==='magic'?'Magic link':'Reset password'}</div>
          </div>
        </div>
        {success ? (
          <div style={{ textAlign:'center', padding:'20px 0' }}>
            ...
            <button onClick={()=>{setSuccess('');setMode('password')}} style={{...lbtn,textAlign:'center',marginTop:16}}>Back to sign in</button>
          </div>
        ) : (
          <form onSubmit={mode==='password'?handlePasswordLogin:mode==='magic'?handleMagicLink:handleReset}>
            ...
            <div style={{ textAlign:'center', marginTop:14 }}>
              {mode!=='password' && <button type='button' onClick={()=>{setMode('password');setError('')}} style={lbtn}>Sign in with password</button>}
              {mode!=='magic' && <button type='button' onClick={()=>{setMode('magic');setError('')}} style={lbtn}>Use magic link instead</button>}
              {mode!=='reset' && <button type='button' onClick={()=>{setMode('reset');setError('')}} style={{...lbtn,color:'#94a3b8'}}>Forgot password?</button>}
            </div>
          </form>
        )}
      </div>
