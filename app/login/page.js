...
export default function LoginPage() {
  ...
  async function handleMagicLink(e) {
    ...
    const { error } = await supabase.auth.signInWithOtp({
      email,
      options: { emailRedirectTo: 'https://miitv-crm.vercel.app/dashboard' }
    })
  ...
  async function handleResetPassword(e) {
    ...
    const { error } = await supabase.auth.resetPasswordForEmail(email, {
      redirectTo: 'https://miitv-crm.vercel.app/dashboard'
    })
    if (error) setError(error.message)
  ...
  if (checking) return <div style={{ minHeight:'100vh', background:'#07090f' }} />
      ...
      <div style={{ width:380, padding:36, background:'rgba(255,255,255,.03)', border:'1px solid rgba(255,255,255,.08)', borderRadius:20 }}>
        ...
        {success ? (
          <div style={{ textAlign:'center', padding:'20px 0' }}>
            ...
            <button onClick={()=>{ setSuccess(''); setMode('password') }} style={{ ...linkBtn, textAlign:'center', color:'#60a5fa', marginTop:16 }}>Back to sign in</button>
          </div>
        ) : (
          <form onSubmit={mode==='password'?handlePasswordLogin:mode==='magic'?handleMagicLink:handleResetPassword}>
            ...
            {mode==='password' && (
              <div style={{ marginBottom:14 }}>
                ...
                <input type='password' required value={password} onChange={e=>setPassword(e.target.value)} placeholder='••••••••' style={box} />
              </div>
            )}
